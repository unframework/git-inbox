...
Repo.prototype.readFile = function (filePath) {
    return this._setupAction().resolvedBy(this._whenCloned.then(function (repo) {
        return loadHeadCommit(repo).then(function (headCommit) { return headCommit.getTree(); }).then(function (headCommitTree) {
            return headCommitTree.entryByPath(filePath).then(function (fileEntry) {
                if (!fileEntry.isFile()) {
                    throw new Error('tree entry is not a file');
                }

                return git.Blob.lookup(repo, fileEntry.oid()).then(function (blob) {
